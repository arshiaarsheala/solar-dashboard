    ...
    with kpi_cols[3]:
        st.markdown("""
            <div class="metric-card">
                ...
                <div class="metric-value">۸,۴۲۹ <span class="metric-unit">میلیارد تومان</span></div>
            </div>
        """, unsafe_allow_html=True)
...
with tab5:
    ...
    st.write("**پیشرفت کل پروژه: 96٪** (فاز پایانی و آماده‌سازی اتصال به شبکه)")
    st.progress(0.96)
    ...
    st.progress(0.20)
    st.caption("اقدامات بحرانی: تمدید مجوز شرکت توزیع برق یزد  و اتصال انشعاب برق کارگاهی اردکان.")
    
    st.markdown("---")
    
    st.markdown("#### 🌐 ساختگاه‌های جدید (فاز توسعه استان یزد)")
    st.write("**پیشرفت مطالعات و اخذ موافقت اصولی: 2٪**")
    st.progress(0.2)
